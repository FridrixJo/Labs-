...
from serializers.constants.json_serializer_constants import INT, FLOAT, BOOL, NONE, STR, VALUE
...
class JSONParser:
    ...
    def inner_dumps(self, value):
        ...
        if isinstance(value, str):
            ...
            return "{" + ", ".join([f"{self.inner_dumps(key)}:{self.inner_dumps(value)}" for key, value in value.items()]) + "}"
    ...
    def inner_loads(self, string):
        ...
        if string[0] == "{" and string[-1] == "}":
            string = string[1:-1]
            result = regex.findall(VALUE, string)
            return {self.inner_loads(result[i][0]): self.inner_loads(result[i + 1][0]) for i in range(0, len(result), 2)}
